...
def _make_terminal_oxt_atom_array(*, covalently_linked: bool = False) -> AtomArray:
    atom_names = ["N", "CA", "C", "O", "OXT"]
    atom_array = AtomArray(len(atom_names) + int(covalently_linked))
    ...
    atom_array.set_annotation(
        "mol_type",
        np.array(["protein"] * 5 + (["ligand"] if covalently_linked else [])),
    )
    atom_array.set_annotation(
        "label_asym_id",
        np.array(["A"] * 5 + (["B"] if covalently_linked else [])),
    )
    ...
    ref_pos = np.array(
        ...
        + ([[0.0, 0.0, 0.0]] if covalently_linked else []),
        dtype=np.float32,
    )
...
def test_save_structure_cif_repairs_each_protein_chain(monkeypatch, tmp_path):
    ...
    for carbon_index, oxt_index in ((2, 4), (7, 9)):
        ...
        assert 0.8 <= c_oxt_distance <= 2.0


def test_save_structure_cif_skips_externally_bonded_terminal_carbon(
    monkeypatch, tmp_path
):
    atom_array = _make_terminal_oxt_atom_array(covalently_linked=True)
...
def test_save_structure_cif_skips_externally_bonded_terminal_oxt(monkeypatch, tmp_path):
    atom_array = _make_terminal_oxt_atom_array(covalently_linked=True)
    atom_array.bonds = BondList(
        len(atom_array),
        np.array([[1, 2, 1], [2, 3, 2], [2, 4, 1], [4, 5, 1]], dtype=np.uint32),
    )
    pred_coordinate = torch.tensor(atom_array.ref_pos + [10.0, 20.0, 30.0])
    pred_coordinate[4] = torch.tensor([100.0, 100.0, 100.0])
    saved = []
